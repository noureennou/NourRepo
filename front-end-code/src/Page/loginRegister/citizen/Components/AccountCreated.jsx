...
export default function AccountCreated({ contact ,userId}){
...
<div className="w-full md:w-[100%] max-w-md p-6 bg-white shadow-xl rounded-xl"style={{marginTop:"25%",marginBottom:"25%"}}>
    ...
    <h4 className="text-lg font-semibold text-blue-800 mb-2-center">User Account Created</h4>   </div>
      ...
      <button
        ...
        className="mt-6 w-full bg-pink-600 hover:bg-pink-700 text-white font-semibold py-2 rounded"onClick={()=>setStep("auth")}
      >
        Proceed to KYC
      </button>
...
{step=="auth" &&<Auth contact={contact} userId={userId} />}

  
      {step === "login" && (
          <Login onBack={() => setStep("done")} />
